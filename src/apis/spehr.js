...
export async function addNewRecord(
  ...
  const tx = SPEHR.methods.addRecord(
    patientBlockchainAddress,
    disease,
    treatment,
    medication,
...
export async function approveRecord(accountAddress, recordDetails) {
  ...
  const tx = SPEHR.methods.approveRecord(recordIDNumeric);
...
export async function declineRecord(accountAddress, recordDetails, declineMsg) {
  ...
  const tx = SPEHR.methods.declineRecord(recordIDNumeric, declineMsg);
  console.log("Transaction: ", tx);
...
export async function newHospitalRegistration(
  ...
  if (!name || !license || !phone || !address || !hospitalBlockchainAddress || !speciality)
  ...
  const tx = SPEHR.methods.doctorRegistration(
    hospitalBlockchainAddress,
    name,
    address,
    phone,
...
export async function newPatientRegistration(
  ...
  const tx = SPEHR.methods.userRegistration(
    patientBlockchainAddress,
    fname,
    lname,
    phone,
...
export function isHospitalAlreadRegistered(hospitalBlockchainAddress) {
  return new Promise((resolve, reject) => {
    SPEHR.methods
      .getDoctorInfo(hospitalBlockchainAddress)
      .call()
      .then((res) => {
