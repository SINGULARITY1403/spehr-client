...
import {
  ...
  DownloadIcon,
  NoteSection,
} from "./HospitalRegistration.styled";
...
const HospitalRegistration = () => {
  ...
  async function goToOTPSection() {
    if (!name || !phone || !license || !address || !speciality) {
  ...
  async function registerHospital() {
    ...
    console.log("New doctor account generated: ", newHospitalAccount);

    let isOccupied = await isHospitalAlreadRegistered(
      newHospitalAccount.address
    );
    if (isOccupied) {
      setIsProcessing(false);
      return alert(
        "The generated address is occupied by another hospital. Reload & try again"
      );
    }
  ...
  function downloadAddress() {
    if (!name || !generatedAccount || !generatedAccount.privateKey) return;
      ...
      <Row>
        ...
        <Col md={12} lg={6}>
          {progress === PROGRESS_STATUSES.DETAILS ? (
            <center>
              <Heading className="my-2">Doctor registration</Heading>
              <br />
              <div>
                ...
                <Checkbox
                  ...
                  required
                />
                Doctor agrees to the Terms and conditions & <br />
            ...
            <div>
              ...
              <div id="receipt-pdf-content">
                <Heading fontSize="36px">
                  Doctor successfuly registered !!
                </Heading>
                <br />
                <ThemeText fontSize="25px">{name}</ThemeText>
                <ThemeText fontSize="18px">{license}</ThemeText>
                <br />
                <br />
